...
def create_model(FLAGS):
    ...
    Bone_features = BoneVector_encoder(ldmk_A_ph, ldmk_B_ph, FLAGS, is_training_ph, bn_decay, reuse=False)
    # Bone_features2 = BoneVector_encoder(ldmk_A_ph+ldmk_B_ph, -ldmk_B_ph, FLAGS, is_training_ph, bn_decay, reuse=True)
    # Bone_features=BoneVector_encoder(pointSet_B_ph, pointSet_A_ph, FLAGS, is_training_ph, bn_decay)
    Bone_motion = BoneMotion_encoder(ldmk_A_ph, ldmk_B_ph, FLAGS, is_training_ph, bn_decay)
    noise1 = None
    ...
    Face_features = FaceVector_encoder(pointSet_A_ph, is_training_ph, noise1, BANDWIDTH, FLAGS, bn_decay=None,reuse=False)
    # Face_features2 = FaceVector_encoder(pointSet_B_ph, is_training_ph, noise1, BANDWIDTH, FLAGS, bn_decay=None,reuse=True)

    s = tf.matmul(hw_flatten(Face_features), hw_flatten(Bone_features), transpose_b=True)  # # [bs, N, N]
    # s2 = tf.matmul(hw_flatten(Face_features2), hw_flatten(Bone_features2), transpose_b=True)
    
    # beta = tf.nn.softmax(s/64, dim=-1)  # attention map
    beta = s/4096
    # beta2 = s2/4096
    # consistency_loss = tf.reduce_mean(tf.squared_difference(beta,beta2))

    o = tf.matmul(beta, Bone_motion)   # [bs, N, N]*[bs, N, c]->[bs, N, c]
    # Pointset_A2B = tf.matmul(beta, ldmk_B_ph)
    displace_A2B = FaceMotion_encoder(pointSet_A_ph, o, FLAGS, is_training_ph, bn_decay)
    # gamma = tf.get_variable("gamma", [1], initializer=tf.constant_initializer(0.0))
    # o = tf.reshape(o, shape=inputs.shape)  # [bs, h, w, C]

    # displace_A2B
    print("PointsetA shape:", pointSet_A_ph.shape, "displace_A2B shape:", displace_A2B.shape)
    
    #Base
    data_loss_A2, shapeLoss_A2, densityLoss_A2, shape_error_A2 = get_Geometric_Loss(pointSet_A_ph, pointSet_B_ph, FLAGS)
    print(data_loss_A2, shapeLoss_A2, densityLoss_A2, shape_error_A2)
#     Predicted_A = pointSet_A_ph
    ...
    # DataLoss = Dataloss_A + consistency_loss
    DataLoss = Dataloss_A
...
# def BoneMotion_encoder(input_pointsA, input_pointsB, FLAGS, is_training, bn_decay=None):
    ...
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='b2layer1')
    ...
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='b2layer2')
    ...
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='b2layer3')
    ...
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='b2layer4')
    ...
    # l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [512, 512], is_training, bn_decay, scope='b2_layer1')
    # l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [512, 256], is_training, bn_decay, scope='b2_layer2')
    # l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [256, 128], is_training, bn_decay, scope='b2_layer3')
    # l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [128, 128, 128], is_training, bn_decay, scope='b2_layer4')
...
# def FaceMotion_encoder(input_pointsA, input_pointsB, FLAGS, is_training, bn_decay=None):
    ...
    # l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=1024, radius=0.1 * FLAGS.radiusScal, nsample=64,
    #                                                    mlp=[64, 64, 128], mlp2=None, group_all=False,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='f2layer1')
    # l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=384, radius=0.2* FLAGS.radiusScal, nsample=64,
    #                                                    mlp=[128, 128, 256], mlp2=None, group_all=False,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='f2layer2')
    # l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=128, radius=0.4* FLAGS.radiusScal, nsample=64,
    #                                                    mlp=[256, 256, 512], mlp2=None, group_all=False,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='f2layer3')

    # # PointNet
    # l4_xyz, l4_points, l4_indices = pointnet_sa_module(l3_xyz, l3_points, npoint=None, radius=None, nsample=None,
    #                                                    mlp=[512, 512, 1024], mlp2=None, group_all=True,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='f2layer4')
    ...
    # ### Feature Propagation layers  #################  featrue maps are interpolated according to coordinate  ################     
    # l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [512, 512], is_training, bn_decay, scope='f2_layer1')
    # l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [512, 256], is_training, bn_decay, scope='f2_layer2')
    # l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [256, 128], is_training, bn_decay, scope='f2_layer3')
    # l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [128, 128, 128], is_training, bn_decay, scope='f2_layer4')
    
    # net = tf_util.conv1d(l0_points, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='f2_fc1', bn_decay=bn_decay )
    # net = tf_util.conv1d(net, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='f2_fc2', bn_decay=bn_decay)
    # net = tf_util.conv1d(net, 3, 1, padding='VALID', activation_fn=None, scope='f2_fc3')
...
def BoneMotion_encoder(input_pointsA, input_pointsB, FLAGS, is_training, bn_decay=None):
    ...
    l0_xyz = input_pointsA
    # l0_points = input_pointsB #Vector
    l0_points = tf.concat([input_pointsA, input_pointsB], axis=-1)

    net = tf_util.conv1d(l0_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='b2_fc1', bn_decay=bn_decay )
    # net = tf_util.conv1d(net, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='b2_fc2', bn_decay=bn_decay)
    # net = tf_util.conv1d(net, 3, 1, padding='VALID', activation_fn=None, scope='f2_fc3')
...
def FaceMotion_encoder(input_pointsA, input_pointsB, FLAGS, is_training, bn_decay=None):
    ...
    net = tf_util.conv1d(l0_points, 3, 1, padding='VALID', activation_fn=None, scope='f2_fc3')

    displacements = tf.sigmoid(net) * FLAGS.range_max * 2 - FLAGS.range_max

    return displacements
    
def BoneVector_encoder(input_pointsA, input_pointsB, FLAGS, is_training, bn_decay=None, reuse=False):

    batch_size = FLAGS.batch_size
    num_points = FLAGS.point_num

    l0_xyz = input_pointsA
    l0_points = input_pointsA #Vector
    # l0_points = tf.concat([input_pointsA, input_pointsB], axis=-1)
    ...
    l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=1024, radius=0.1 * FLAGS.radiusScal, nsample=64,
                                                       mlp=[64, 64, 128], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='blayer1',reuse=reuse)
    l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=384, radius=0.2* FLAGS.radiusScal, nsample=64,
                                                       mlp=[128, 128, 256], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='blayer2',
                                                       reuse=reuse)
    l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=128, radius=0.4* FLAGS.radiusScal, nsample=64,
                                                       mlp=[256, 256, 512], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='blayer3',
                                                       reuse=reuse)

    # PointNet
    l4_xyz, l4_points, l4_indices = pointnet_sa_module(l3_xyz, l3_points, npoint=None, radius=None, nsample=None,
                                                       mlp=[512, 512, 1024], mlp2=None, group_all=True,
                                                       is_training=is_training, bn_decay=bn_decay, scope='blayer4',
                                                       reuse=reuse)
    ...
    ### Feature Propagation layers  #################  featrue maps are interpolated according to coordinate  ################     
    l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [512, 512], is_training, bn_decay, scope='b_layer1',reuse=reuse)
    l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [512, 256], is_training, bn_decay, scope='b_layer2',reuse=reuse)
    l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [256, 128], is_training, bn_decay, scope='b_layer3',reuse=reuse)
    l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [128, 128, 128], is_training, bn_decay, scope='b_layer4',reuse=reuse)
    
    # l0_points = tf.concat([input_pointsA, l0_points], axis=-1)
    l0_points = tf_util.conv1d(l0_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='att_fc1', bn_decay=bn_decay )
    ##### Attention for bony encoder
    # l3_points = tf.expand_dims(l3_points,axis=2)
    # l3_points = attention_unit(l3_points, is_training=is_training) 
...
def FaceVector_encoder(input_points, is_training, noise, sigma, FLAGS, bn_decay=None, weight_decay=None,reuse=True):
    ...
    l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=1024, radius=0.1 * FLAGS.radiusScal, nsample=64,
                                                       mlp=[64, 64, 128], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='flayer1',
                                                       reuse=reuse)
    l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=384, radius=0.2* FLAGS.radiusScal, nsample=64,
                                                       mlp=[128, 128, 256], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='flayer2',
                                                       reuse=reuse)
    l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=128, radius=0.4* FLAGS.radiusScal, nsample=64,
                                                       mlp=[256, 256, 512], mlp2=None, group_all=False,
                                                       is_training=is_training, bn_decay=bn_decay, scope='flayer3',
                                                       reuse=reuse)

    # PointNet
    l4_xyz, l4_points, l4_indices = pointnet_sa_module(l3_xyz, l3_points, npoint=None, radius=None, nsample=None,
                                                       mlp=[512, 512, 1024], mlp2=None, group_all=True,
                                                       is_training=is_training, bn_decay=bn_decay, scope='flayer4',
                                                       reuse=reuse)
    ...
    ### Feature Propagation layers  #################  featrue maps are interpolated according to coordinate  ################     
    l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [512, 512], is_training, bn_decay, scope='f_layer1',reuse=reuse)
    l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [512, 256], is_training, bn_decay, scope='f_layer2',reuse=reuse)
    l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [256, 128], is_training, bn_decay, scope='f_layer3',reuse=reuse)
    l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [128, 128, 128], is_training, bn_decay, scope='f_layer4',reuse=reuse)

    # l0_points = tf.concat([point_cloud, l0_points], axis=-1)
    l0_points = tf_util.conv1d(l0_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='att_fc2', bn_decay=bn_decay )
    # ## Attention model
    # l0_points = tf.expand_dims(l0_points,axis=2)
    # l0_points = attention_unit(l0_points, is_training=is_training)
...
# def get_distance_mask(inputpoints1, inputpoints2, FLAGS):
...
#     bs = FLAGS.batch_size
#     npts = FLAGS.point_num
#     R = FLAGS.radiusScal *0.2

#     square_dist1 = pairwise_l2_norm2_batch(inputpoints1, inputpoints2)
#     dist1 = tf.sqrt(square_dist1+1e-12) #[batch_size, npts, npts]
#     #dist1 = square_dist1
#     mask = tf.zeros_like(dist1)
#     mask[dist1 < R] = 1
#     return mask